...
namespace synth
{
	...
	{
		FTYPE dFreq = w(dHertz) * dTime + dLFOAmplitude * sin(w(dLFOHertz) * dTime);// osc(dTime, dLFOHertz, OSC_SINE);
		...
		case OSC_SAW_DIG: // Saw Wave (optimized / harsh / fast)
			return 2.0 * dHertz * fmod(dTime, 1.0 / dHertz) - 1.0;

		case OSC_NOISE: // Pseudo Random Noise
			return 2.0 * (static_cast<FTYPE>(rand()) / static_cast<FTYPE>(RAND_MAX)) - 1.0;
